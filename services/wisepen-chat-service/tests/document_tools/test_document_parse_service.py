...
from chat.application.tools.document_tools.document_parse.parsers.generic import (
    generic_document_parser as generic_document_module,
)
from chat.application.tools.document_tools.document_parse.parsers.generic.generic_document_parser import (
...
async def test_service_routes_supported_spreadsheets_to_pandas_without_common_fallback(
    ...
    monkeypatch.setattr(service_module, "GenericDocumentParser", _UnexpectedCommonParser)
...
async def test_service_does_not_route_unsupported_excel_formats_to_pandas(
    ...
    monkeypatch.setattr(service_module, "GenericDocumentParser", _CommonParser)
...
async def test_common_parser_uses_markitdown_only_after_docling_failure(
    ...
    monkeypatch.setattr(generic_document_module, "DoclingParser", _FailingDoclingParser)
    monkeypatch.setattr(generic_document_module, "MarkItDownParser", _MarkItDownParser)

    result = await GenericDocumentParser().parse(DocumentParseRequest(file_path="sample.txt"))
