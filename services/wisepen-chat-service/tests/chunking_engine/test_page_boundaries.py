...
def test_markdown_pipeline_keeps_page_boundaries_as_chunk_boundaries() -> None:
    ...
    assert result.chunks[0].metadata["page_range"] == ("1", "1")
...
def test_nested_markdown_parent_chunks_do_not_cross_pages_after_finalizer_merges() -> None:
    ...
    assert parents[0].metadata["page_range"] == ("1", "1")
    assert parents[1].metadata["page_range"] == ("2", "2")
    assert "<!-- page 2 -->" not in parents[0].text
    assert "<!-- page 1 -->" not in parents[1].text


def test_nested_markdown_child_chunks_inherit_parent_page_range() -> None:
    page_one = "第一页内容。" * 120
    page_two = "第二页内容。" * 120
    text = "\n\n".join(
        (
    ...
    for child in children:
        ...
        assert child.metadata["page_range"] == parent.metadata["page_range"]
        assert child.metadata["page_numbers"] == parent.metadata["page_numbers"]
