...
def parse_equation(
    ...
    if "=" in equation and "==" not in equation:
        ...
        return sp.Eq(
            parse_expr(left, variables, extra_locals=extra_locals),
            parse_expr(right, variables, extra_locals=extra_locals),
        )
    parsed = parse_expr(equation, variables, extra_locals=extra_locals)
...
def parse_inequality(expression: str | None, variable: str) -> Relational:
    """解析一元不等式表达式。"""
    parsed = parse_expr(expression, [variable])
...
def parse_bound(value: str | None, name: str, variables: Iterable[str] | None = None) -> sp.Expr:
    """解析积分、求和或数值区间边界。"""
    try:
        return parse_expr(value, variables)
