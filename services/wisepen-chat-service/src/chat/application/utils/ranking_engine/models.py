...
class ScoreSignalKind(StrEnum):
    ...
    DIVERSITY = "diversity"  # 多样性 / 去重信号，例如 MMR、near dedup、group suppression
...
class RankQuery:
    ...
    metadata: Metadata = field(default_factory=dict)  # 调用方附加元数据，ranking engine 不解释其含义
...
@dataclass(frozen=True, slots=True)
class RankCandidate:
    """排序候选对象，Ranking Engine 只认识这个标准输入形态。"""
...
class ScoreSignal:
    ...
    reason: str = ""  # 信号解释文本
    metadata: Metadata = field(default_factory=dict)  # 信号附加元数据，用于调试、解释和追踪
