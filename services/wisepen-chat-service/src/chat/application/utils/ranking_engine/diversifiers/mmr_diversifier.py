...
class MmrDiversifierConfig:
    ...
    top_k: int | None = None  # None 表示使用当前 ranked 全量
...
class MmrDiversifier:
    ...
    def diversify(
        ...
        top_k = len(ranked) if cfg.top_k is None else min(max(cfg.top_k, 0), len(ranked))
        if top_k <= 0:
            return ()
        ...
        while len(selected_items) < top_k and len(selected_ids) < len(ranked):
        ...
        selected_id_set = {item.candidate_id for item in selected_items}
        tail = [item for item in ranked if item.candidate_id not in selected_id_set]
        ordered = tuple(selected_items + tail)

        return assign_ranks(ordered)
