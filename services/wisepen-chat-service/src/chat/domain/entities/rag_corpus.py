...
class RagChunkLocatorDocument(BaseModel):
    locator_name: str = Field(..., description="完整定位名")
    locator_kind: LocatorKind = Field(..., description="定位类型")
    start_offset: int | None = Field(
        default=None, description="定位覆盖的原文起始 offset"
    )
    end_offset: int | None = Field(
        default=None, description="定位覆盖的原文结束 offset"
    )
...
class RagParentChunkDocument(Document):
    ...
    locators: list[RagChunkLocatorDocument] = Field(
        default_factory=list, description="定位项"
    )
