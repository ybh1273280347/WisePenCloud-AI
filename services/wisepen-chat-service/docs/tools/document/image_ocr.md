...
`file_ref` 和 `file_path` 必须二选一。`file_ref` 通过 `ToolRunFileStore.resolve_ref(...)` 校验 `user_id/session_id` 作用域；URL 形式的 `file_path` 使用工具层共享 URL fetcher 下载。
...
file_path URL
  -> tools/utils/url_fetcher.HttpxFetcher
  -> temp file
  -> MIME check
  -> OCR client
  -> cacheable_texts
...
- URL 下载能力复用 `tools/utils/url_fetcher.py`，不依赖 `web_fetch` 私有 fetcher。
