...
> 第一次读？建议按下面顺序：
> 1. 先读 [toolchain_architecture](toolchain_architecture.md)，理解 `ToolReturn`、`cnt_*`、`file_*`、URL 缓存和模型约束。
...
| 统一切面 | 是否经过 `ToolReturn`、`ToolOutputCache`、`ToolContentStore`、`FileReferenceStore`、`web_content_cache` 或 GC |
