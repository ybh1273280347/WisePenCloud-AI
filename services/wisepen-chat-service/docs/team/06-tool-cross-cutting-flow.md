...
3. **拆分 schema 与 execute 校验**：类型、枚举、required、范围放 JSON Schema；one-of、条件参数、权限、manifest、跨字段业务语义放 custom preflight 或工具 `execute()` 入口。
...
```text
platform_search/exa_search/... -> url -> web_fetch -> cnt_* -> session read
...
- 搜索候选直接暴露 URL；需要正文时交给 `web_fetch(urls=[...])`。
- 文件移交用 `tfile_*`，不传本地路径。
- 大文本用 `cnt_*`，不反复抓取或解析。
