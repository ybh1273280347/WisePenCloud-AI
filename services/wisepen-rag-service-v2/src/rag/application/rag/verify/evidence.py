...
async def verify_candidates(
    ...
    if len(resource_ids) != 1 or len(revisions) != 1:
        raise EvidenceRevisionError("evidence candidates must share one resource revision")
    resource_id = next(iter(resource_ids))
    content_revision = next(iter(revisions))
    records = await reader.read_applied_evidence(
        resource_id,
        content_revision,
...
def _verify_candidate(candidate: EvidenceCandidate, record: EvidenceRecord) -> None:
    source_ref = record.source_ref
    chunk = candidate.chunk
    if source_ref.resource_id != candidate.resource_id:
        raise EvidenceRevisionError(f"source ref {source_ref.ref_id} has invalid resource")
    if source_ref.content_revision != candidate.content_revision:
        raise EvidenceRevisionError(f"source ref {source_ref.ref_id} has invalid revision")
    if source_ref.ref_id != candidate.source_ref_id:
        raise EvidenceCorruptError("evidence reader returned a mismatched source ref")
    if chunk.chunk_id != source_ref.chunk_id:
        raise EvidenceCorruptError(f"chunk {chunk.chunk_id} does not match source ref")
    if chunk.reading_block_id != source_ref.reading_block_id:
        raise EvidenceCorruptError(f"chunk {chunk.chunk_id} has invalid reading block")
    if chunk.section_id != source_ref.section_id:
        raise EvidenceCorruptError(f"chunk {chunk.chunk_id} has invalid section")
    if chunk.section_path != source_ref.section_path:
        raise EvidenceCorruptError(f"chunk {chunk.chunk_id} has invalid section path")
    if chunk.source_spans != source_ref.source_spans:
        raise EvidenceCorruptError(f"chunk {chunk.chunk_id} has invalid source spans")
    if chunk.page_labels != source_ref.page_labels:
        raise EvidenceCorruptError(f"chunk {chunk.chunk_id} has invalid page labels")
    if chunk.anchor_labels != source_ref.anchor_labels:
        raise EvidenceCorruptError(f"chunk {chunk.chunk_id} has invalid anchor labels")
    if record.reading_block.block_id != source_ref.reading_block_id:
        raise EvidenceCorruptError(f"source ref {source_ref.ref_id} has invalid block record")
    if record.reading_block.section_id != source_ref.section_id:
        raise EvidenceCorruptError(f"source ref {source_ref.ref_id} has invalid block owner")
    if record.section.section_id != source_ref.section_id:
        raise EvidenceCorruptError(f"source ref {source_ref.ref_id} has invalid section record")
    if chunk.raw_text != record.source_text:
        raise EvidenceCorruptError(f"chunk {chunk.chunk_id} does not match authoritative text")
